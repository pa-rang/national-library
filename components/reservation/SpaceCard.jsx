...
const SpaceCard = ({ roomNumber, reservationStatus, isPossible }) => {
	return (
		<SpaceCardWrapper isPossible={isPossible}>
			<div className="card__number">{roomNumber}</div>
			<div className="card__info">
				<div className="card__info__status">{isPossible ? '이용가능' : '이용 불가'}</div>
				<div className="card__info__people">수용인원: 1명</div>
				<div className="card__info__time-blocks">
					{reservationStatus.map((val, idx) => (
						<TimeBlock key={idx} isReserved={val} time={idx + 10} />
					))}
				</div>
			</div>
		</SpaceCardWrapper>
